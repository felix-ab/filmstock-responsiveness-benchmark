...
def build_doc() -> None:
    ...
    subtitle.add_run("A filmstock-based benchmark comparing ChatGPT Image 2 and Grok Imagine across 288 generated images")
    ...
    add_para(doc, "The empirical result is comparative rather than a single model ranking: photographic prompt responsiveness is multidimensional. Lighting is the strongest and most reliable prompt factor, filmstock and scan are detectable but subtler, and the most robust model difference appears as a model-by-lighting interaction. Blocked PERMANOVA on PC1 through PC10 found significant effects for lighting, model-by-lighting, model, scan, filmstock, and filmstock-by-lighting. Hierarchical bootstrap inference confirmed a significant model difference for warm-practical lighting responsiveness but not for the filmstock or scan contrasts. The conclusion is that a filmmaker's benchmark should evaluate not only correctness, but the controllability of aesthetic dimensions that matter in professional creative workflows.")

    add_heading(doc, "1. Filmstock Responsiveness as a Benchmarking Problem", 1)
    add_para(doc, "This study begins from a practical creative problem. In applied image-generation workflows, a model can be technically impressive while still offering weak aesthetic control. It may render a plausible person, a readable object, and a convincing scene, yet respond only faintly to the language that photographers, cinematographers, designers, and art directors use to shape a specific look. A prompt like 'a woman wearing a red shirt riding a bicycle' tests a model's ability to place objects and attributes. A prompt that adds 'CineStill 800T,' 'warm tungsten practical lights,' or 'pushed one stop in development' tests a different capability: whether the model understands the visual consequences of a production-style photographic instruction.")
    add_para(doc, "For that reason, the study is intentionally about photographic prompt responsiveness rather than ordinary prompt adherence. In applied AI product discussions, models are often compared through informal judgments about taste, practical controllability, and whether the system can be steered toward a specific visual result. Those impressions matter, but they are rarely translated into a reproducible statistical design. This study turns one part of that intuition into a controlled experiment: explicit image features, matched prompt contrasts, multivariate tests, robust-estimation sensitivity analyses, and visual examples.")
    ...
    add_para(doc, "This study shows that photographic prompt responsiveness can be studied statistically. The benchmark generated 288 images, extracted 195 image features, constructed blocked prompt-response distances, and analyzed the results with paired tests, hierarchical bootstrap inference, PCA, blocked MANOVA, blocked PERMANOVA, regression, WLS, Huber robust regression, Kruskal-Wallis and Friedman tests, PLS, LASSO/ridge classification, variance decomposition, and robust outlier diagnostics.")
    ...
    add_para(doc, "Future work should expand the filmmaker's benchmark in several directions: more models, more scenes, more repeats per condition, human ratings from photographers and art directors, reference sets of photographs shot on the named film stocks, open-model comparisons with controlled seeds and sampling steps, and additional cinematic variables such as lens focal length, camera height, staging density, production design, wardrobe palette, time of day, and color grade. The broader goal is to evaluate generative models as tools for artistic control, not only as systems for plausible image completion.")
